...
KeepCols <- c("shots.made","shots.attempted","free.throws.made"
              ,"points.scored","points.allowed","Playoff.Win")
prc <- prc[,KeepCols]
prc

#Rename Classifier column values
table(prc$Playoff.Win)
prc$Playoff.Win <- factor(prc$Playoff.Win, levels = c("1", "0"), labels = c("Win at Playoffs", "Don't"))
table(prc$Playoff.Win)
...
prc_train <- prc_n[1:316,]
prc_test <- prc_n[317:343,]

#Including Classifier labels
prc_train_labels <- prc[1:316, 6]
prc_test_labels <- prc[317:343, 6]
